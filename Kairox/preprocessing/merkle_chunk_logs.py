...
def hash_node(left: int, right: int) -> int:
    """Matches a standard two-input hash logic (like Pedersen or Poseidon)"""
    return (left + right) % FELT_MOD

def hash_leaf_cairo(line: str, index: int) -> int:
    """Convert line + index into Cairo-friendly felt"""
    # Use only the first 31 bytes to ensure it fits in a single Felt comfortably
    # or hash the string first. To be safe and simple:
    val = 0
    for char in line[:31]: # Cairo shortstrings are max 31 chars
        val = (val << 8) + ord(char)
    val = val % FELT_MOD
    return (val + index) % FELT_MOD # Matches Cairo (left + right)

# -----------------------------
# Merkle tree construction (Binary Tree)
# -----------------------------
def build_merkle_root(leaves: list[int]) -> int:
    """Builds a binary Merkle tree and returns the root."""
    if not leaves:
        return 0
    ...
    while len(nodes) > 1:
        ...
        for i in range(0, len(nodes), 2):
            left = nodes[i]
            # If there's no right sibling, we promote the left or hash with itself
...
def main():
    ...
    log_name = os.path.splitext(os.path.basename(log_file))[0]
    log_chunks_dir = os.path.join("chunks", log_name)
    os.makedirs(log_chunks_dir, exist_ok=True)
    ...
    # 1. Compute line hashes
    print("Computing Cairo-compatible line hashes...")
    with Pool(cpu_count()) as pool:
        line_hashes = pool.map(hash_leaf_worker, [(i, line) for i, line in enumerate(lines)])
    ...
    print("\n--- Processing Chunk Merkle Roots ---")
    for chunk_id, chunk_lines, start_idx in chunks:
        # Extract the line hashes for this specific chunk
        chunk_line_hashes = line_hashes[start_idx : start_idx + len(chunk_lines)]
        
        # Calculate Chunk Merkle Root
        c_root = build_merkle_root(chunk_line_hashes)
        chunk_roots.append(c_root)
        
        print(f"Chunk {chunk_id}: Root = {hex(c_root)}")
        
        chunk_data_storage.append({
            "id": chunk_id,
            "lines": chunk_lines,
            "start": start_idx,
            "root": c_root
    ...
    # 3. Calculate Global Root (Merkle root of the chunk roots)
    full_root = build_merkle_root(chunk_roots)
    print("\n--- Final Results ---")
    print(f"GLOBAL MERKLE ROOT: {hex(full_root)}")

    # 4. Save to files
    for chunk in chunk_data_storage:
        chunk_dir = os.path.join(log_chunks_dir, f"chunk_{chunk['id']}")
        os.makedirs(chunk_dir, exist_ok=True)
        ...
        with open(os.path.join(chunk_dir, "meta.json"), "w") as f:
            json.dump({
                ...
                "end_line": chunk['start'] + len(chunk['lines']) - 1,
                "chunk_root": str(chunk['root']),
                "full_root": str(full_root)
            }, f, indent=4)

    print(f"\nSuccess! Metadata saved in {log_chunks_dir}")
